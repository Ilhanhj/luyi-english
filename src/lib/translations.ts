export const translations = {
  id: {
    nav: {
      home: "Home",
      tutors: "Tutor",
      pricing: "Harga",
      faq: "FAQ",
      contact: "Kontak",
      chat: "Chat Sekarang",
      openMenu: "Buka menu",
    },
    tutorsPage: {
      title: "Temukan Mentor Terbaikmu",
      subtitle: "Pilih dari puluhan tutor berpengalaman yang siap membantumu lancar berbahasa Inggris dengan metode personal.",
      searchPlaceholder: "Cari nama tutor...",
      filters: {
        all: "Semua",
        ielts: "IELTS & TOEFL",
        business: "Business",
        kids: "Kids",
        conversation: "Conversation",
        grammar: "Grammar",
      },
      card: {
        students: "Siswa",
        experience: "Pengalaman",
        bookBtn: "Booking Kelas",
        profileBtn: "Lihat Profil",
      },
      cta: {
        title: "Masih Bingung Memilih?",
        subtitle: "Konsultasikan kebutuhan belajarmu dengan admin kami. Kami akan merekomendasikan tutor yang paling pas buat kamu.",
        btn: "Konsultasi Gratis",
      },
    },
    hero: {
      // UPDATE: Copywriting lebih hooking & Trust Badge
      trustBadge: "⭐ Platform Belajar Paling Recommended",
      title: "Jago Inggris Tanpa Bosan. Hasil Nyata, Karir Melesat.",
      subtitle: "Lupakan metode kuno. Kami gunakan pendekatan personal yang membuat Anda percaya diri berbicara sejak hari pertama. Siap mendunia?",
      cta: {
        viewTutors: "Lihat Tutor Kami",
        freeTrial: "Daftar",
      },
      stats: {
        students: "Siswa Terdaftar",
        tutors: "Tutor Profesional",
        satisfaction: "Tingkat Kepuasan",
        rating: "Rating Rata-Rata",
      },
    },
    tutors: {
      title: "Tutor Profesional Kami",
      subtitle: "Belajar dari para ahli yang berdedikasi untuk kesuksesanmu.",
      specializations: {
        business: "Business English",
        ielts: "IELTS & TOEFL Prep",
        conversation: "Conversational English",
        kids: "English for Kids",
        grammar: "Grammar & Writing",
        publicSpeaking: "Public Speaking",
      },
    },
    testimonials: {
      title: "Apa Kata Mereka?",
      subtitle: "Cerita nyata dari para siswa yang belajar bersama LuyiEnglish",
      cta: "Lihat Semua Testimoni",
    },
    testimonialsGallery: {
      title: "Galeri Testimoni ❤️",
    },
    pricing: {
      title: "Investasi untuk Masa Depanmu",
      subtitle: "Belajar bukan sekadar biaya — ini langkah menuju versi terbaik dirimu.",
      perMonth: "",
      perProgram: "",
      cta: "Pilih Paket",
      badges: {
        bestValue: "Paling Hemat",
        maxFocus: "Fokus Maksimal",
        mostPopular: "Paling Populer",
        dreamScore: "Skor Impian",
        goInternational: "Go International",
      },
      features: {
        meetings12: "12x pertemuan/bulan",
        flexibleSchedule: "Jadwal ditentukan bersama",
        max15: "Maks. 15 siswa/ruang",
        max6: "Maks. 6 siswa/ruang",
        minutes60: "60 menit/pertemuan",
        dailyMaterial: "Materi Bahasa Inggris Harian",
        oneOnOne: "Tutor 1-on-1",
        moduleIncluded: "Modul termasuk",
        prePostTest: "Pre-test & Post-test",
      },
      packages: {
        group: { title: "Kelas Grup", description: "Belajar seru bareng teman-teman baru." },
        semiPrivate: { title: "Kelas Semi Privat", description: "Kelompok kecil, perhatian lebih personal." },
        private: { title: "Kelas Privat", description: "Fokus penuh pada tujuan belajarmu." },
        toefl: { title: "Persiapan TOEFL", description: "Raih skor TOEFL impianmu dengan bimbingan ahli." },
        ielts: { title: "Kelas IELTS", description: "Siap taklukkan tes IELTS dengan strategi jitu." },
      },
    },
    faq: {
      title: "Pertanyaan yang Sering Diajukan",
      subtitle: "Tidak menemukan jawabanmu? Hubungi kami langsung.",
      items: [
        {
          id: "faq1",
          question: "Kelas apa saja yang tersedia?",
          answer: "Kami menyediakan berbagai kelas Bahasa Inggris seperti Speaking, Grammar, Writing, dan Vocabulary. Kamu bisa pilih sesuai dengan tujuan belajar dan level kemampuanmu.",
        },
        {
          id: "faq2",
          question: "Sistem kelasnya seperti apa?",
          answer: "Kelas diadakan secara online melalui Zoom dengan konsep interaktif dan santai. Setiap sesi dipandu langsung oleh tutor (MR) yang bikin suasana belajar tetap seru dan mudah dipahami.",
        },
        {
          id: "faq3",
          question: "Biaya kursusnya berapa?",
          answer: "Biaya tergantung pada jenis kelas dan durasinya. Tapi tenang, semua program kami dibuat supaya terjangkau dan sebanding dengan manfaatnya. Rincian lengkap akan diberikan setelah kamu memilih kelas yang diinginkan.",
        },
        { id: "faq4", question: "Apakah bisa bayar secara cicilan?", answer: "Bisa banget! Kami menyediakan opsi pembayaran fleksibel termasuk sistem cicilan, jadi kamu bisa mulai belajar tanpa harus bayar penuh di awal." },
        {
          id: "faq5",
          question: "Bagaimana cara daftar kelasnya?",
          answer: "Mudah banget! Setelah kamu menentukan program yang cocok, tim admin kami akan bantu langkah demi langkah untuk proses pendaftaran. Kamu juga bisa langsung daftar lewat chat atau form di website kami.",
        },
      ],
    },
    contact: {
      title: "Jangan Tunggu Lagi, Kuasai Bahasa Inggris Sekarang!",
      subtitle: "Ambil langkah pertama menuju masa depan yang lebih cerah. Hubungi kami untuk sesi trial gratis.",
      cta: "Chat Sekarang",
    },
    footer: {
      tagline: "Belajar Bahasa Inggris dengan cara yang menyenangkan dan mudah bersama tutor profesional kami.",
      quickLinks: "Tautan Cepat",
      contactUs: "Hubungi Kami",
      followUs: "Ikuti Kami",
      rights: "Semua hak dilindungi.",
    },
  },
  en: {
    nav: {
      home: "Home",
      tutors: "Tutors",
      pricing: "Pricing",
      faq: "FAQ",
      contact: "Contact",
      chat: "Chat Us Now",
      openMenu: "Open menu",
    },
    tutorsPage: {
      title: "Find Your Best Mentor",
      subtitle: "Choose from dozens of experienced tutors ready to help you master English with a personalized method.",
      searchPlaceholder: "Search tutor name...",
      filters: {
        all: "All",
        ielts: "IELTS & TOEFL",
        business: "Business",
        kids: "Kids",
        conversation: "Conversation",
        grammar: "Grammar",
      },
      card: {
        students: "Students",
        experience: "Experience",
        bookBtn: "Book Class",
        profileBtn: "View Profile",
      },
      cta: {
        title: "Still Confused?",
        subtitle: "Consult your learning needs with our admin. We will recommend the perfect tutor for you.",
        btn: "Free Consultation",
      },
    },
    hero: {
      // UPDATE: Hooking Copy & Trust Badge
      trustBadge: "⭐ The Most Recommended English Learning Platform",
      title: "Master English Without Boredom. Real Results, Career Boost.",
      subtitle: "Forget outdated methods. We use a personal approach that makes you confident to speak from day one. Ready to go global?",
      cta: {
        viewTutors: "View Our Tutors",
        freeTrial: "Sign Up for Free Trial",
      },
      stats: {
        students: "Registered Students",
        tutors: "Professional Tutors",
        satisfaction: "Satisfaction Rate",
        rating: "Average Rating",
      },
    },
    tutors: {
      title: "Our Professional Tutors",
      subtitle: "Learn from experts dedicated to your success.",
      specializations: {
        business: "Business English",
        ielts: "IELTS & TOEFL Prep",
        conversation: "Conversational English",
        kids: "English for Kids",
        grammar: "Grammar & Writing",
        publicSpeaking: "Public Speaking",
      },
    },
    testimonials: {
      title: "What They Say",
      subtitle: "Real stories from students who learn with LuyiEnglish",
      cta: "View All Testimonials",
    },
    testimonialsGallery: {
      title: "Testimonials Gallery ❤️",
    },
    pricing: {
      title: "An Investment in Your Future",
      subtitle: "Learning isn't just a cost — it's a step toward the best version of yourself.",
      perMonth: "",
      perProgram: "",
      cta: "Choose Package",
      badges: {
        bestValue: "Best Value",
        maxFocus: "Maximum Focus",
        mostPopular: "Most Popular",
        dreamScore: "Dream Score",
        goInternational: "Go International",
      },
      features: {
        meetings12: "12 meetings/month",
        flexibleSchedule: "Jointly determined schedule",
        max15: "Max. 15 students/class",
        max6: "Max. 6 students/class",
        minutes60: "60 minutes/meeting",
        dailyMaterial: "Daily English Material",
        oneOnOne: "1-on-1 Tutor",
        moduleIncluded: "Module included",
        prePostTest: "Pre-test & Post-test",
      },
      packages: {
        group: { title: "Grouping Class", description: "Fun learning with new friends." },
        semiPrivate: { title: "Semi Private Class", description: "Small group, more personal attention." },
        private: { title: "Private Class", description: "Full focus on your learning goals." },
        toefl: { title: "TOEFL Preparation", description: "Achieve your dream TOEFL score with expert guidance." },
        ielts: { title: "IELTS Class", description: "Ready to conquer the IELTS test with smart strategies." },
      },
    },
    faq: {
      title: "Frequently Asked Questions",
      subtitle: "Can't find your answer? Contact us directly.",
      items: [
        { id: "faq1", question: "What classes are available?", answer: "We offer a variety of English classes such as Speaking, Grammar, Writing, and Vocabulary. You can choose based on your learning goals and proficiency level." },
        {
          id: "faq2",
          question: "What is the class system like?",
          answer: "Classes are held online via Zoom with an interactive and relaxed concept. Each session is guided by a tutor (MR) who keeps the learning atmosphere fun and easy to understand.",
        },
        {
          id: "faq3",
          question: "How much does the course cost?",
          answer: "The cost depends on the type of class and its duration. But rest assured, all our programs are designed to be affordable and worth the benefits. Full details will be provided after you choose your desired class.",
        },
        { id: "faq4", question: "Can I pay in installments?", answer: "Absolutely! We offer flexible payment options including an installment system, so you can start learning without having to pay in full upfront." },
        {
          id: "faq5",
          question: "How do I sign up for a class?",
          answer: "It's super easy! Once you decide on a suitable program, our admin team will assist you step-by-step through the registration process. You can also register directly via chat or the form on our website.",
        },
      ],
    },
    contact: {
      title: "Don't Wait, Master English Now!",
      subtitle: "Take the first step towards a brighter future. Contact us for a free trial session.",
      cta: "Chat Us Now",
    },
    footer: {
      tagline: "Learn English the fun and easy way with our professional tutors.",
      quickLinks: "Quick Links",
      contactUs: "Contact Us",
      followUs: "Follow Us",
      rights: "All rights reserved.",
    },
  },
};
