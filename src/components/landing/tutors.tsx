...
import { TutorModal } from "@/components/ui/tutor-modal"; // Import Modal
...
export default function Tutors() {
  ...
  const handleTutorClick = (tutor: any) => {
    // Tambahkan properti dummy agar sesuai dengan kebutuhan TutorModal
    const formattedTutor = {
      ...tutor,
      rating: 5.0,
      students: "50+",
      experience: tutor.experience || "3 Tahun",
    };
    setSelectedTutor(formattedTutor);
    setIsModalOpen(true);
  };
    ...
    <section id="tutors" className="py-20 md:py-32 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6 relative">
        <div className="text-center mb-12" data-aos="fade-up">
          ...
          <SectionSubtitle className="mt-4 text-slate-400">{t.tutors.subtitle}</SectionSubtitle>
        </div>
              ...
              {tutorsData.map((tutor) => (
                <CarouselItem key={tutor.id} className="pl-4 md:pl-6 md:basis-1/2 lg:basis-1/3">
                  <div className="h-full pt-2 pb-2">
                    {/* Pembungkus Card - Ditambah onClick dan kursor pointer */}
                    <div
                      onClick={() => handleTutorClick(tutor)}
                      className="group relative flex flex-col h-full rounded-3xl overflow-hidden transition-all duration-300 hover:-translate-y-2 bg-slate-900/40 backdrop-blur-xl border border-white/10 hover:border-amber-500/30 hover:shadow-2xl hover:shadow-amber-500/10 cursor-pointer"
                    >
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="relative shrink-0">
                            ...
                            <Image
                              ...
                              width={80}
                              height={80}
                              unoptimized={true} // <-- SOLUSI BLUR DI SINI
                              className="relative rounded-2xl object-cover border-2 border-white/10 group-hover:border-transparent transition-all duration-300 shadow-lg h-20 w-20"
                            />
                            {/* Verified Icon */}
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="font-bold text-lg text-white truncate group-hover:text-amber-400 transition-colors">{tutor.name}</h3>
                            <p className="text-sm text-blue-200/80 font-medium truncate mb-2">{tutor.role}</p>
                      ...
                      <CardContent className="p-6 flex flex-col flex-grow relative z-10">
                        {/* Bio */}
                        <p className="text-sm text-slate-300 leading-relaxed mb-6 line-clamp-3">{tutor.bio}</p>

                        {/* Review Section */}
                        <div className="mt-auto relative">
                          <Quote className="absolute -top-4 -left-2 w-8 h-8 text-amber-500/20 rotate-180" />

                          <div className="bg-gradient-to-br from-white/10 to-transparent rounded-2xl p-4 border border-white/5 relative backdrop-blur-sm group-hover:bg-white/5 transition-colors">
                            <p className="text-sm italic text-slate-200 relative z-10 leading-relaxed line-clamp-4">"{tutor.testimonials[0]?.quote || "No testimonial yet."}"</p>
                            <div className="flex items-center justify-end gap-2 mt-3 pt-3 border-t border-white/5">
                              ...
                              <p className="text-xs font-bold text-amber-400 uppercase tracking-wide truncate max-w-[150px]">{tutor.testimonials[0]?.author || "Student"}</p>
                            </div>
                          </div>
                        </div>
                      </CardContent>
      ...
      {/* COMPONENT MODAL DIPASANG DI SINI */}
