...
export default function Faq() {
  return (
    <section id="faq" className="py-16 md:py-24 bg-transparent relative">
      <div className="absolute top-0 left-0 w-full h-full bg-cover bg-center opacity-10 dark:opacity-5" style={{backgroundImage: 'url(/noise.png)'}}></div>
          ...
          <Accordion type="single" collapsible className="w-full space-y-4">
            {faqItems.map((item) => (
              <AccordionItem key={item.id} value={item.id} className="glass-card overflow-hidden !border-0 shadow-sm transition-all duration-300 hover:border-pink-500/30 hover:shadow-xl hover:-translate-y-1">
                <AccordionTrigger className="text-left font-semibold text-lg hover:no-underline px-6 py-4 text-foreground hover:bg-secondary/50 transition-colors">
                  {item.question}
                </AccordionTrigger>
