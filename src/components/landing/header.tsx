...
const LanguageSwitcher = ({ isMobile = false }: { isMobile?: boolean }) => {
  ...
  return (
    <div className={cn("flex items-center p-1 rounded-full border border-white/5", isMobile ? "bg-slate-900/50 w-fit mx-auto mt-6" : "bg-slate-950/30 backdrop-blur-sm")}>
      {isMobile ? null : <Globe className="w-3.5 h-3.5 text-slate-500 ml-2 mr-1" />}
      <button onClick={() => setLanguage("id")} className={cn(baseStyle, language === "id" ? activeStyle : inactiveStyle)}>
        ID
      </button>
      <button onClick={() => setLanguage("en")} className={cn(baseStyle, language === "en" ? activeStyle : inactiveStyle)}>
        EN
      </button>
    </div>
  );
      ...
      >
        <Link href="/" className="flex items-center gap-2 relative z-50 group">
          <Image
            ...
            className={cn("object-contain transition-all duration-500", isScrolled ? "w-28 md:w-32" : "w-32 md:w-40")}
          />
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {navLinks.map((link) => {
            ...
            const finalHref = link.href.startsWith("#") && !isHome ? `/${link.href}` : link.href;

            // Logic Active State:
            // Kalau di Home -> Cek activeLink state
            // Kalau di Tutors -> Cek pathname startWith link.href (biar /tutors tetep nyala)
            const isActive = isHome ? activeLink === link.href : link.href !== "#home" && pathname.startsWith(link.href);
            ...
            return (
              <Link
                ...
                className={cn("relative px-4 py-2 text-sm font-medium transition-all duration-300 rounded-full group overflow-hidden", isActive ? "text-white" : "text-slate-400 hover:text-white")}
              >
                <span className={cn("absolute inset-0 bg-white/10 rounded-full transition-all duration-300", isActive ? "opacity-100" : "opacity-0 group-hover:opacity-100")}></span>

                {isActive && <span className="absolute bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 bg-amber-400 rounded-full shadow-[0_0_5px_rgba(251,191,36,1)]"></span>}
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="bg-slate-950/95 backdrop-blur-3xl border-l-white/10 w-[320px] p-0 overflow-hidden sm:max-w-sm">
              ...
              <SheetHeader className="p-8 border-b border-white/5 text-left">
                ...
                <Image src="/images/logo-luyi-v2.png" alt="LuyiEnglish" width={140} height={40} unoptimized={true} className="w-32 object-contain opacity-90" />
              </SheetHeader>
              ...
              <nav className="flex flex-col p-6 space-y-2">
                {navLinks.map((link, i) => {
                  ...
                  return (
                    ...
                    >
                      {link.label}
                      <ChevronRight className={cn("w-5 h-5 transition-all duration-300 text-amber-400", isActive ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0")} />
                    </Link>
                  );
                })}
