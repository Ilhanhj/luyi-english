...
import { MoveRight, Users, Star, MapPin, MessageCircle, Send, Globe } from "lucide-react";
...
export default function Hero() {
  ...
  // Jangan lupa pastikan icon ini udah di-import dari lucide-react di atas file ya:

  const stats = [
    // Item 1: Wide (Span 2) - TIKTOK & MASSIVE STUDENTS HOOK
    {
      id: "tiktok",
      icon: Users,
      // Value dibikin pendek & nge-hook karena ini teks yang paling besar ukurannya
      value: "#1 Choice",
      label: language === "id" ? "English Course Terpopuler di TikTok! 🔥" : "Most Popular English Course on TikTok! 🔥",
      color: "text-blue-400",
      span: "md:col-span-2",
    },
    // Item 2: Small - THE GLOBAL HOOK
    {
      id: "global",
      icon: Globe,
      value: "15+",
      label: language === "id" ? "Negara Asal Siswa" : "Student Countries",
      color: "text-cyan-400",
      span: "md:col-span-1",
    },
    // Item 3: Small - THE METHOD HOOK
    {
      id: "method",
      icon: MessageCircle,
      value: "80%",
      label: language === "id" ? "Praktek Ngobrol" : "Speaking Practice",
      color: "text-purple-400",
      span: "md:col-span-1",
    },
    // Item 4: Small - TRUST HOOK
    {
      id: "rating",
      icon: Star,
      value: "4.9/5",
      label: language === "id" ? "Rating Kepuasan" : "Satisfaction Rate",
      color: "text-yellow-400",
      span: "md:col-span-1",
    },
    // Item 5: Small - TUTOR HOOK
    {
      id: "tutors",
      icon: Sparkles,
      value: "100%",
      // Menekankan kalau tutornya gak cuma jago, tapi juga asik
      label: language === "id" ? "Tutor Seru & Ahli" : "Fun & Expert Tutors",
      color: "text-amber-400",
      span: "md:col-span-1",
    },
    // Item 6: Wide (Span 2) - GLOBAL DIVERSITY HOOK
    {
      id: "diversity",
      icon: MapPin,
      // Dibuat lebih ringkas biar rapi di mobile
      value: language === "id" ? "Lintas Benua 🌍" : "Global Reach 🌍",
      label: language === "id" ? "Dipercaya siswa dari Asia, Australia, hingga Eropa." : "Trusted by students across Asia, Aus, & Europe.",
          ...
          <div className="space-y-8 flex flex-col items-center" data-aos="fade-up">
            ...
            <div className="relative inline-block">
              ...
              <h1 className="relative z-10 text-4xl font-extrabold tracking-tighter sm:text-5xl md:text-6xl lg:text-7xl leading-[1.1]">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-amber-200 to-amber-500 drop-shadow-sm">{t.hero.title}</span>
              </h1>
            ...
            <p className="mx-auto max-w-[750px] text-slate-300 md:text-xl font-medium leading-relaxed">{t.hero.subtitle}</p>
            ...
            <div className="flex flex-col sm:flex-row gap-5 justify-center pt-6 w-full sm:w-auto">
              <Button
                ...
                variant="outline"
                size="lg"
                className="glass-card border-white/10 text-white backdrop-blur-md rounded-full h-14 px-8 text-lg font-semibold transition-all duration-300 hover:bg-white/10 hover:border-white/30 hover:scale-105 w-full sm:w-auto"
              >
                <Link href="#tutors">
              ...
              <Button
                asChild
                size="lg"
                className="relative bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold rounded-full h-14 px-8 text-lg shadow-[0_0_25px_rgba(37,99,235,0.4)] transition-all duration-300 hover:scale-105 hover:shadow-[0_0_35px_rgba(37,99,235,0.6)] hover:-translate-y-1 border-0 w-full sm:w-auto overflow-hidden"
              >
                <Link
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-5 max-w-6xl mx-auto">
              {stats.map((stat, index) => (
                ...
                >
                  ...
                  <div className={cn("absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-500", stat.color.replace("text-", "bg-"))} />

                  {/* Blob Dekorasi di pojok */}
                  <div className={cn("absolute -top-12 -right-12 w-24 h-24 rounded-full blur-3xl opacity-20 group-hover:opacity-40 transition-all duration-500", stat.color.replace("text-", "bg-"))} />

                  <div className="relative z-10 flex flex-col items-center justify-center p-6 text-center h-full">
                    {/* Icon Box */}
                    <div className={cn("mb-3 p-3 rounded-2xl border border-white/5 shadow-inner transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3", "bg-slate-950/30")}>
                      <stat.icon className={cn("h-7 w-7 md:h-8 md:w-8", stat.color)} strokeWidth={2} />
                    </div>

                    {/* Value */}
                    <p className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight drop-shadow-sm leading-none mt-1">{stat.value}</p>

                    {/* Label */}
                    <p className="text-xs sm:text-sm text-slate-400 font-bold uppercase tracking-widest mt-3 group-hover:text-white transition-colors">{stat.label}</p>
                  </div>
                </div>
              ))}
            </div>
