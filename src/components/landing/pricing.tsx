...
function PricingCard({ pkg }: { pkg: ReturnType<typeof getPricingPackages>[0] }) {
  return (
    <div className="relative pt-4 h-full">
      {" "}
      {/* Tambah padding top buat tempat badge */}
      <div
        className={cn(
          ...
          pkg.isPopular ? "ring-2 ring-amber-400/50 shadow-amber-500/20" : "hover:border-white/20 hover:shadow-blue-500/10",
        )}
      >
        ...
        <div
          className={cn(
            ...
            pkg.isPopular ? "bg-gradient-to-b from-amber-500/20 to-transparent" : "bg-gradient-to-b from-blue-500/10 to-transparent",
          )}
        />
          ...
          <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-50 w-max">
            <Badge
              className={cn(
                ...
                pkg.isPopular ? "bg-gradient-to-r from-amber-400 to-yellow-500 text-slate-900 hover:from-amber-500 hover:to-yellow-600" : "bg-blue-600 text-white",
              )}
            >
        ...
        <CardHeader className="p-6 text-center relative z-10">
          <CardTitle className={cn("text-lg font-bold tracking-tight", pkg.isPopular ? "text-amber-300" : "text-blue-200")}>{pkg.title}</CardTitle>

          {/* <div className="flex items-baseline justify-center gap-1 pt-4">
            <span className="text-xs font-medium text-slate-400 self-start mt-2"></span>
            <span
              className={cn(
          ...
          <ul className="space-y-3 flex-grow mb-6">
            {pkg.features.map((feature, i) => (
              <li key={i} className="flex items-start gap-2 group/item">
                <div className={cn("mt-0.5 rounded-full p-0.5 flex-shrink-0", pkg.isPopular ? "bg-amber-400/20 text-amber-400" : "bg-emerald-500/20 text-emerald-400")}>
                  <Check className="h-3 w-3" strokeWidth={3} />
                </div>
                <span className="text-slate-300 text-xs font-medium group-hover/item:text-white transition-colors text-left leading-tight">{feature.name}</span>
              </li>
            ))}
          </ul>
    ...
    <section id="pricing" className="py-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative">
        <div className="text-center mb-12 max-w-3xl mx-auto" data-aos="fade-up">
          ...
          <SectionSubtitle className="mt-4 text-slate-400">{t.pricing.subtitle}</SectionSubtitle>
        </div>
        ...
        */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mt-12" data-aos="fade-up" data-aos-delay="200">
